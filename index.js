...
<div class='info-item'>
  ...
  <p>${g}</p>
                ...
                `).join("");p.innerHTML=o,b.refresh()}const w=document.querySelector(".js-search"),p=document.querySelector(".gallery"),a=document.querySelector(".js-load-more"),m=document.querySelector(".loader");let f="",c=1,u=15;w.addEventListener("submit",r=>{if(r.preventDefault(),p.innerHTML="",f=r.target.elements.search.value.trim(),f===""){i.error({title:"Error",message:"Please enter a search query"});return}c=1,a.classList.add("load-more-hidden"),h()});const P=()=>{const r=document.querySelector(".gallery-item");if(r){const o=r.getBoundingClientRect().height;window.scrollBy({top:o*2,behavior:"smooth"})}},h=async()=>{try{m.style.display="block";const r=await S(f,c,u);if(m.style.display="none",r.hits.length===0){i.info({message:"Sorry, no results found. Try again."}),a.classList.add("load-more-hidden");return}q(r.hits),P(),c+=1,r.hits.length<u||c>Math.ceil(r.totalHits/u)?(a.classList.add("load-more-hidden"),i.info({message:"We are sorry, but you have reached the end of search results."})):a.classList.remove("load-more-hidden")}catch{m.style.display="none",a.classList.add("load-more-hidden"),i.error({title:"Error",message:"Failed to load images"})}};a.addEventListener("click",h);
